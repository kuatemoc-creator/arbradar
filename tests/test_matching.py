...
def test_windfall_tax_in_the_eu_is_one_lead_whatever_the_headline():
    a = match._topics_of([_it("EU Considers Windfall Tax on Energy Companies Amid Oil Price Surge")])
    b = match._topics_of([_it("'Oil companies are exploiting the situation': Europe eyes windfall tax on energy giants")])
    c = match._topics_of([_it("Germany Pushes EU To Tax Oil Windfall Profits Tied To Hormuz")])
    assert a & b and a & c


def test_a_czech_windfall_tax_is_another_lead():
    eu = match._topics_of([_it("EU Considers Windfall Tax on Energy Companies")])
    cz = match._topics_of([_it("Czech government reinstates fuel margin caps, plans windfall tax on refineries")])
    assert not (eu & cz)


def test_a_story_with_no_measure_has_no_topic():
    assert match._topics_of([_it("Egypt defeats Saudi real estate investors’ mega-claim")]) == set()


def test_entities_ignore_dictionary_words_and_states():
    ents = match._entities("Turkey revokes Iran’s Bank Mellat license after 44 years")
    assert "mellat" in ents
    assert "turkey" not in ents and "license" not in ents


def test_a_capital_names_its_state():
    prague = match._topics_of([_it("Prague caps fuel prices and taxes refiners' windfall profits")])
    czech = match._topics_of([_it("Czech government reinstates fuel margin caps, plans windfall tax on refineries")])
    assert prague & czech
...
def test_clustering_keeps_the_copies_a_story_already_has():
    ...
    out = match.cluster([rep, _it("Unrelated mining licence revoked in Ghana", event_type="state_measure")])
    assert len(out) == 2
